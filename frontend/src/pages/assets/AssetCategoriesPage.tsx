...
export function AssetCategoriesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (body: { name: string; description?: string }) =>
      api.post<{ success: true; data: Category }>('/asset-categories', body).then((r) => r.data),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-dark-text">Asset Categories</h1>
        <button
          type="button"
      ...
      {!isLoading && (
        <div className="rounded-xl border border-gray-200 dark:border-dark-border bg-white dark:bg-dark-card overflow-hidden">
          ...
          {list.length === 0 && (
            <p className="p-6 text-gray-500 dark:text-dark-textSecondary text-center">No categories yet. Add one to get started.</p>
          )}
        </div>
      )}
    </div>
